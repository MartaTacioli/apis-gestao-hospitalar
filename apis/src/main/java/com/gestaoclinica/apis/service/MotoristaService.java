...
public class MotoristaService {
	...
	public String criarLoginMotorista(RequestLoginMotorista obj) throws IOException{
		...
		if (objAprovaMotoAntes.getAprovado() == 0) {
		...
		OkHttpClient client = new OkHttpClient().newBuilder()
				...
				Request request = new Request.Builder()
				  .url("https://qa.portaldotransportador.com/externo/api/ws/alocaja/new_motorista/post")
				  .method("POST", body)
				  .addHeader("apiKey", "78asd4546d4sa687e1d1xzlcknhwyhuWMKPSJDpox8213njdOWnxxipW58547")
				  .addHeader("Cookie", "ci_session=6d75q7dr6nlnq2ntu06g539gsfjvsqra")
				  .build();
				...
				ResponseLoginMotorista responseLogin = new ResponseLoginMotorista();
				Gson gson = new Gson();
				
				String jsonString = gson.toJson(response);
				System.out.println("RESPONSE 2"+ response.toString());
				responseLogin = gson.fromJson(response.toString(), ResponseLoginMotorista.class);

				System.out.println("responseLogin 2"+ responseLogin.toString());
				if (responseLogin.isSuccess() == true) {
					...
					return "Conta criada com sucesso! Username:" + responseLogin.getUsername() + " e senha: " + responseLogin.getPassword();
				} else {
					System.out.println("ELSES");
					return responseLogin.getMessage();
	...
	public Motorista atualizarMotorista(Motorista obj){
		try {
	...
	public AnuncioResumoCard findAllProjectedByIdServico(Long id) {
		AnuncioResumoCard obj = repository.findProjectedById(id);
		return obj;
	}
	*/
	...
	public List<AnunciosFiltrados> filtrar (AnuncioFiltro obj) {
		...
		if (obj.getCidade().length() >0 ) {
			if (obj.getTipoImovel() == null) {
				
				List<AnunciosFiltrados> anunciosFiltrados1 = repository.findAllProjectedByBanheirosGreaterThanAndDormitoriosGreaterThanAndVagasGreaterThanAndAreaPrivadaGreaterThanAndValorImovelLessThanAndRegiaoCidade(obj.getBanheiros(), obj.getDormitorios(), obj.getVagas(), obj.getAreaPrivada(), obj.getValorImovel(), obj.getCidade());
				return anunciosFiltrados1;
	
			} else {
				
				List<AnunciosFiltrados> anunciosFiltrados1 = repository.findAllProjectedByTipoImovelAndBanheirosGreaterThanAndDormitoriosGreaterThanAndVagasGreaterThanAndAreaPrivadaGreaterThanAndValorImovelLessThanAndRegiaoCidade(obj.getTipoImovel(), obj.getBanheiros(), obj.getDormitorios(), obj.getVagas(), obj.getAreaPrivada(), obj.getValorImovel(), obj.getCidade());
				return anunciosFiltrados1;
			...
			if (obj.getRegiao().isEmpty()) {
				if (obj.getTipoImovel() == null) {
					List<AnunciosFiltrados> anunciosFiltrados2 = repository.findAllProjectedByBanheirosGreaterThanAndDormitoriosGreaterThanAndVagasGreaterThanAndAreaPrivadaGreaterThanAndValorImovelLessThan(obj.getBanheiros(), obj.getDormitorios(), obj.getVagas(), obj.getAreaPrivada(), obj.getValorImovel());
					return anunciosFiltrados2;
				} else {
					List<AnunciosFiltrados> anunciosFiltrados2 = repository.findAllProjectedByTipoImovelAndBanheirosGreaterThanAndDormitoriosGreaterThanAndVagasGreaterThanAndAreaPrivadaGreaterThanAndValorImovelLessThan(obj.getTipoImovel(),obj.getBanheiros(), obj.getDormitorios(), obj.getVagas(), obj.getAreaPrivada(), obj.getValorImovel());
					return anunciosFiltrados2;
				}
			} else { 
				if (obj.getTipoImovel() == null) {
				List<AnunciosFiltrados> anunciosFiltrados2 = repository.findAllProjectedByBanheirosGreaterThanAndDormitoriosGreaterThanAndVagasGreaterThanAndAreaPrivadaGreaterThanAndValorImovelLessThanAndRegiaoIn(obj.getBanheiros(), obj.getDormitorios(), obj.getVagas(), obj.getAreaPrivada(), obj.getValorImovel(), obj.getRegiao());
				return anunciosFiltrados2;
					
				} else {
				List<AnunciosFiltrados> anunciosFiltrados2 = repository.findAllProjectedByTipoImovelAndBanheirosGreaterThanAndDormitoriosGreaterThanAndVagasGreaterThanAndAreaPrivadaGreaterThanAndValorImovelLessThanAndRegiaoIn(obj.getTipoImovel(),obj.getBanheiros(), obj.getDormitorios(), obj.getVagas(), obj.getAreaPrivada(), obj.getValorImovel(), obj.getRegiao());
				return anunciosFiltrados2;
				}
			}
	...
	*/
	...
	public Motorista insert (Motorista obj) {	
		...
		try {
			...
			//e.printStackTrace();
	       throw new ReferenciaExternaException(obj.getId().toString());
			...
			//e.printStackTrace();
			throw new CamposObrigatoriosException (obj, e.getLocalizedMessage());
		
		} catch (InvalidDataAccessApiUsageException e) {
			//System.out.println("errinho");
			//e.printStackTrace();
			throw new ReferenciaExternaException (e.getLocalizedMessage());
	...
	public Motorista insertCliente (Motorista obj) {	
		System.out.println("entrou111");
			Corretor corr = obj.getCorretor();
			Proprietario prop = obj.getProprietario();
			System.out.println("ENTROU2222");

			System.out.println("o proprietario existe"+ repositoryProprietario.existsById(prop.getCpf()));
			...
			if (!repositoryCorretor.existsById(corr.getCpf())) {
				...
				throw new NaoExisteEsseCorretorException ("Esse CPF de corretor não existe em nossa base. Converse com seu corretor ou selecione a opção 'Ainda não tenho um corretor da IMBECKER'!");

			}
			
			if (repositoryProprietario.existsById(prop.getCpf()) && (!(obj.getProprietario().getEmail() == null) || !(obj.getProprietario().getTelefone() == null) || !(obj.getProprietario().getNome()==null)) ){
			
				System.out.println("PROPRIETARIOSERVICE: Não é o primeiro anuncio para esse cpf");
					throw new NaoEPrimeiroAnuncioException ("Não é o primeiro anúncio para esse CPF.");
			}else if(repositoryProprietario.existsById(prop.getCpf())){
				System.out.println("ENTROU777");

				try {
					System.out.println("ENTROU888");
		       ...
		       throw new AccessApiUsageException(obj.getId().toString());
				...
				try {
					...
					repositoryProprietario.save(prop);
					System.out.println("ENTRANDO 888111");
					repository.save(obj);
			       ...
			       throw new AccessApiUsageException(obj.getId().toString());
					...
					//System.out.println("erro 1");
					//e.printStackTrace();
					throw new FaltaDadosProprietarioException ("Como é o primeiro anúncio nesse CPF, é necessário ativar a função 'É meu primeiro imóvel na IMBECKER' e cadastrar o nome, telefone e email do proprietário.");
			...
			System.out.println("RETURN OBJETO:" + obj);
			return obj;
