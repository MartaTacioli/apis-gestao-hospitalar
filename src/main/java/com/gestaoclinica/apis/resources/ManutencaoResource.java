...
public class ManutencaoResource {
	...
	@GetMapping(value = "/usuario-dono/{cpf}")
	public ResponseEntity<List<Manutencao>> findAllByDono(@PathVariable Long cpf){
		List<Manutencao> obj = service.findAllByDono(cpf);
		return ResponseEntity.ok().body(obj);
	}
	...
	@PostMapping(value = "/realizar-manutencao")
	public ResponseEntity<Manutencao> realizarManutencao (@RequestBody Manutencao obj){
		obj = service.realizarManutencao(obj);
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				  .buildAndExpand(obj).toUri();
		return ResponseEntity.created(uri).body(obj);
	}
