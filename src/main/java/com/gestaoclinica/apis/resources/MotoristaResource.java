...
import com.gestaoclinica.apis.service.MotoristaService;
...
public class MotoristaResource {
	...
	@PostMapping (value="/aprovar-motorista")
	public String criarLoginMotorista (@RequestBody RequestLoginMotorista obj) throws IOException{
		return service.criarLoginMotorista(obj);
	}
	
	@PostMapping (value="/consultar-aprovacao")
	public int consultarAprovacao (@RequestBody RequestLoginMotorista obj) throws IOException{
		return service.consultarAprovacao(obj);
	}
	
	
	
	@PutMapping(value = "/atualizar")
	public ResponseEntity<Motorista> atualizarMotorista (@RequestBody Motorista obj){
		System.out.println("entrou no put pelo menos");
		obj = service.atualizarMotorista(obj);
	...
	public ResponseEntity<AnuncioResumoCard> findAllProjectedByIdServico(@PathVariable Long id){
		System.out.println("ENTROU 1");
		AnuncioResumoCard obj = service.findAllProjectedByIdServico(id);
	...
	*/
	...
	public ResponseEntity<List<AnunciosFiltrados>> filtrar (@RequestBody AnuncioFiltro obj){
		System.out.println("resource insert:");
		System.out.println(obj.toString());
		List<AnunciosFiltrados> listaAnuncios = service.filtrar(obj);
		

		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				  .buildAndExpand(obj.getClass()).toUri();
		return ResponseEntity.created(uri).body(listaAnuncios);
	}
	/*
	...
	public ResponseEntity<Motorista> insertCliente (@RequestBody Motorista obj){
